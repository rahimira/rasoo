...
from youto import handle_youtube_download  # اضافه شد
...
async def get_weather(city):
    try:
        url = (
            "https://api.codesazan.ir/Weather/"
            "?key=7832824256:QMkb9158atit@CodeSazan_APIManager_Bot"
        ...
        debug_print(f"weather error: {e}")
    return None

def format_weather(result):
    return (
        f"🌤️ وضعیت آب‌وهوا برای {result.get('address', '')}\n"
        ...
        f"وضعیت: {result.get('weather_conditions','')}\n"
        f"🌡️ دما: {result.get('degree','')}\n"
...
async def handle(client, m: Message):
    ...
    match_weather = re.match(r"^هواشناسی\s+(.+)$", text)
    if match_weather:
        city = match_weather.group(1).strip()
        await m.edit("⏳ در حال دریافت اطلاعات آب‌وهوا برای " + city + " ...")
        weather_data = await get_weather(city)
        if weather_data and weather_data.get("status") == 200 and "result" in weather_data:
            weather_text = format_weather(weather_data["result"])
            await m.edit(weather_text)
        else:
            await m.edit("❌ دریافت اطلاعات آب‌وهوا ممکن نشد یا شهر یافت نشد.")
        return

    if text in {"تاریخ", "date", "امروز"}:
        try:
            beautiful_date = get_beautiful_date()
            await m.edit(beautiful_date)
        except Exception as e:
            await m.edit(f"❌ خطا در دریافت تاریخ: {e}")
        return

    if m.reply_to_message and m.reply_to_message.text and (text in {"ترجمه", "translate", "ترجمه کن"}):
        orig = m.reply_to_message.text.strip()
        translated = await translate_text(orig, "fa")
        if translated:
            await m.edit(translated)
        else:
            await m.edit("⚠️ خطا در ترجمه")
        return

    match = re.match(r"^ترجمه\s+([a-zA-Zآ-ی]+)\s+(.+)$", text)
    if match:
        lang_word = match.group(1).strip().lower()
        orig = match.group(2).strip()
        to_lang = LANG_MAP.get(lang_word, "fa")
        if not orig and m.reply_to_message and m.reply_to_message.text:
            orig = m.reply_to_message.text
        if not orig:
            await m.edit("❌ متنی برای ترجمه پیدا نشد.")
            return
        translated = await translate_text(orig, to_lang)
        if translated:
            await m.edit(translated)
        else:
            await m.edit("⚠️ خطا در ترجمه")
    ...
    if m.from_user and m.from_user.is_self:
        if text in {".help", "/help"}:
            ...
            try:
                if m.text != HELP_TEXT.format(mention=mention):
